...
const minNumber = filterArgs(Math.min, isNumber);
const minInteger = filterArgs(minNumber, Number.isInteger);
const minFinite = filterArgs(minNumber, isFinite);
const isNegative = filterArgs(isNumber, (num) => num < 0);
const maxNegativeInteger = filterArgs(maxNumber, isNegative, Number.isInteger);
...
process.env.SELF_TEST && (() => {
    ...
    assert(Number.isNaN(nan), "NaN is returned, If any argument cannot be converted to a number, .");
